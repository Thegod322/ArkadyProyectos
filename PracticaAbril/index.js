...
function resolveMaterialImage(materialCard) {
    ...
    if (inlineMaterialImage) {
        return inlineMaterialImage;
    }
...
function setActiveSlidePreviewState(isVisible) {
    ...
    gsap.to(current.visual, {
        opacity: isVisible ? 0.08 : 1,
        duration: 0.32,
        ease: 'power2.out',
        overwrite: true
    });
    ...
    const hide = () => {
        ...
        gsap.to(materialPreview, {
            ...
            onComplete: () => {
                ...
                gsap.set(materialPreview, { autoAlpha: 0 });
            }
        });
    };

    if (delay > 0) {
        previewHideTimer = window.setTimeout(hide, delay);
        return;
    }
...
function showMaterialPreview(image) {
    ...
    if (!materialPreviewImage || materialPreviewImage === image) {
        reveal();
        return;
    }
...
function showActiveSlide(index, animateContent) {
    ...
    slideParts.forEach((parts, partsIndex) => {
        if (partsIndex !== index) {
            resetSlide(parts);
        }
    });
...
function buildTransition(direction) {
    ...
    gsap.set(target.visual, { scale: direction > 0 ? 1.08 : 0.96, opacity: 0.18 });
    ...
    tl.to(current.visual, {
        scale: direction > 0 ? 1.14 : 0.94,
        opacity: 0,
        duration: 1,
        ease: 'none'
    }, 0);

    tl.to(current.overlay, {
        opacity: 0.15,
        duration: 1,
        ease: 'none'
    }, 0);
...
function finishTransition(commit) {
    ...
    gsap.to(activeTransition.timeline, {
        progress: commit ? 1 : 0,
        duration: commit ? 0.42 : 0.48,
        ease: commit ? 'power3.out' : 'power2.out',
        overwrite: true,
        onComplete: () => {
            activeTransition.timeline.kill();

            if (commit) {
                activeIndex = activeTransition.toIndex;
            }
...
function onWheel(event) {
    ...
    if (isMaterialPreviewVisible) {
        hideMaterialPreview();
    }

    if (isSettling || slides.length < 2) {
        return;
    }
    ...
    transition.distance = gsap.utils.clamp(
        0,
        SCROLL_DISTANCE,
        transition.distance + (delta * transition.direction)
    );
